...
export const getPosts = createAsyncThunk("/posts", async (query: any) => {
  ...
  //   console.log(typeof posts)here
  let data = {};
  if (query.search) {
    data = {
      count: posts.filter(
        (item: any) =>
          item.title.slice(0, query.search.length).toLowerCase() ==
    ...
    data = {
      count: posts.length,
      data: posts.slice(
        query.dropdown * (query.page - 1),
        query.page * query.dropdown
      ),
    };
  } else {
    data = {
      count: posts.filter((item: any) => item.status == query.currentStatus)
        .length,
      data: posts
        .filter((item: any) => item.status == query.currentStatus)
        .slice(query.dropdown * (query.page - 1), query.page * query.dropdown),
    };
  }
  console.log(data);
